 ...
 * Time: 14:43
 */
public class GeographyMatcherTest extends TestCase {

    private GeographyMatcher cut;

    private Building b1, b3;
    private CityModel cityModel1, cityModel3;
    ...
    public void setUp() {
        ...
        Matching matching = new Matching(head1, head3);
        distanceTable.put(matching, 0);
        cut = new GeographyMatcher(distanceTable);

    }

    public void testMatchCityModels() {
        DistanceTable distanceTable = cut.matchCityModels(head1, head3, new HashMap<String[], Boolean>());
        assertNotNull(distanceTable);

        for (Matching matching : distanceTable.keySet()) {
            AbstractTreeNode node1 = matching.getNode1();
            AbstractTreeNode node2 = matching.getNode2();
            if (node1 instanceof ElementNode && node2 instanceof ElementNode) {
                Object value1 = ((ElementNode) node1).getValue();
                Object value2 = ((ElementNode) node2).getValue();
                if (value1 instanceof AbstractGML && value2 instanceof AbstractGML) {
                    String id1 = ((AbstractGML) value1).getId();
                    String id2 = ((AbstractGML) value2).getId();
...
//        assertNotNull(distanceTable.getMatching("GEOM_6398777_0_:GEOM_6398777_0__SHIFTED"));
    }
